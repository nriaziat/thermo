...
import numpy as np
from AdaptiveID import DeflectionAdaptation
...
for tip_therm, tip_aruco, therm_frame in zip(therm_defl, aruco_defl, therm_frames):
    ...
    if defl_adapt is None:
        defl_adapt = DeflectionAdaptation(np.array([tip_therm_mm[0], tip_therm_mm[1], 0, 0, tip_therm_mm[0], tip_therm_mm[1], 1, 2]),
                                          labels=['x', 'y', 'xd', 'yd', 'ry', 'rz', 'c_defl', 'k_tissue'])
        aruco_adapt = DeflectionAdaptation(np.array([tip_aruco[0], tip_aruco[1], 0, 0, tip_aruco[0], tip_aruco[1], 1, 2]),
                                          labels=['x', 'y', 'xd', 'yd', 'ry', 'rz', 'c_defl', 'k_tissue'])
    ...
    cv.putText(color_frame, f"Estimated Values: {defl_adapt.kf.x[6]:.2f}, {defl_adapt.kf.x[7]:.2f}", (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
ax[1].scatter(aruco_defl, filtered_therm_defl, marker='.', label='Filtered Thermal Signal', alpha=0.5)
x = np.linspace(0, max(aruco_defl), 25)
ax[1].plot(x, x, 'g--', label='y=x')
ax[1].plot(x, linreg.slope * x + linreg.intercept, 'r', label=r'$R^2={{{:.2f}}}$'.format(linreg.rvalue**2))
