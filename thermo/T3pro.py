...
def u16(m3: np.ndarray, idx: int) -> int:
...
def sub_10001180(fpatmp_: float, coretmp_: float, cx: float) -> np.ndarray:
    ...
    part_Tatm_Trefl = (1.0 - Emiss_) * t * (refltmp_ - ABSOLUTE_ZERO_CELSIUS) ** 4 + (1.0 - t) * (
    ...
    if debug > 1:
        ...
        print('cx:', cx, 'v2:', v2)
        print('v5:', v5)
        print('flt_1000339C', flt_1000339C, 'flt_10003398', flt_10003398, 'flt_10003394', flt_10003394, 'fpatmp_',
              fpatmp_)
        print('v22:', v22)
        print('v23:', v23)
        print('np1:', v[:10])
...
def temperatureLut(fpatmp_: float, meta3: np.ndarray) -> np.ndarray:
...
def device_info(meta: np.ndarray, meta_mapping: int = 3, idx: int = 48) -> list[str]:
...
class T3pro:
    ...
    def read_(self) -> tuple[bool, np.ndarray, np.ndarray, np.ndarray]:
