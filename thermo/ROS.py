...
from geometry_msgs.msg import Pose, Point32, Point # for position and velocity of ASTR
...
class PointCloudPublisher(Node):
    ...
    def set_command(self, traj: Trajectory):
        for i in range(len(traj)):
            pt = Point32()
            pt.x = traj.points[i][0]
            pt.y = traj.points[i][1]
            pt.z = traj.points[i][2]
            self.pc.points.append(pt)

    def publish_command(self):
        self.publisher_.publish(self.pc)

def CommandTFPublisher(Node):
    def __init__(self):
        ...
        self.tf.child_frame_id = 'electrocautery_arm_base_link'
        self.loop_rate = self.create_rate(100)
    ...
    def publish_command(self):
        self.publisher_.publish(self.tf)
...
class AstrCartesianCommandPublisher(Node):
    ...
    def set_command(self, pose: Pose, velocity: float, tf: TransformStamped):
        ...
        self.command.motion_mode.requested_ang_vel_deg_s = 5.
