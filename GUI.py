...
class ControlExperimentUI(TKMT.ThemedTKinterFrame):
    ...
    def create_widgets(self):
        ...
        self.model_type_frame.Radiobutton("MPC", variable=self.model_name, value="pseudostatic")
        self.model_type_frame.Radiobutton("Minimization", variable=self.model_name, value="minimization")
        self.model_type_frame.Radiobutton("Constant Velocity", variable=self.model_name, value="constant_velocity")
        ...
        self.numerical_params_frame.Entry(textvariable=self.r)
    ...
    def model_selection(self):
        if self.model_name.get() == "minimization" or self.model_name.get() == "constant_velocity":
    ...
    def start_experiment(self):
        ...
        main(model, mpc, exp_type, adaptive_velocity, constant_velocity, save_dir, self.logFile.get(), t3, tb, self.r.get())
        if t3 is not None:
            t3.release()
        if tb is not None:
            tb.stop()
